...
class GameOutCard extends AStrategy
{
    ...
    public function exec()
    {
        ...
        return $this->playCard($user_room_data, $out_cards, $account);
    }
    ...
    protected function playCard($user_room_data, $out_cards, $account)
    {
        ...
        if ($out_cards['status'] == 1) {
            if (count($out_cards['card']) == 0) {
                return $this->gameOutCard(['status' => 1, 'msg' => '出牌非法, 请出手牌']);
            }
            //判断手牌是否存在, 手牌存在继续往下执行
            if (! $out_cards['card'] == array_intersect($out_cards['card'], $my_card['card'])) {
                return $this->gameOutCard(['status' => 2, 'msg' => '出牌非法, 出牌数据有问题']);
            }
            //检查牌型
            $arr = $this->obj_ddz->checkCardType($out_cards['card']);
            if ($arr['type'] == 0) {
                return $this->gameOutCard(['status' => 3, 'msg' => '出牌非法, 牌型有误']);
            }
            $card_type = $arr['type_msg'];

            //如果非首轮牌, 请验证牌型, 并判断牌型是否一直, 如果打出的牌型是, 炸弹和飞机, 跳过验证, 13表示炸弹,14表示飞机
            if ($last_card_type > 0 && ! in_array($arr['type'], [13, 14]) && $last_card_type != $arr['type']) {
                return $this->gameOutCard(['status' => 3, 'msg' => '出牌非法, 和上一把牌型不符合']);
            }
            $out_cards['card_type'] = $arr['type'];
            //比牌大小
            if (! $this->obj_ddz->checkCardSize($out_cards['card'], json_decode($last_card, true))) {
                return $this->gameOutCard(['status' => 4, 'msg' => '出牌非法, 牌没有大过上家牌']);
            }
        } else {
            //过牌要验证是否为首次出牌, 如果是首次出牌是不能过牌的
            if ($hand == 1 || $last_chair_id == $out_cards['chair_id']) {
                return $this->gameOutCard(['status' => 4, 'msg' => '出牌非法, 首次出牌不能过牌操作']);
            }
        }
        if ($out_cards['chair_id'] < 1) {
            return $this->gameOutCard(['status' => 5, 'msg' => '出牌非法, 椅子ID非法']);
        }
        //判断游戏是否结束
        if (count($my_card['card']) < 1) {
            return $this->gameOutCard(['status' => 6, 'msg' => '游戏结束, 所有手牌已经出完']);
        }
...
//        var_dump($next_card, $prv_card, $is_out_card);
    ...
    protected function gameOutCard($param)
    {
        ...
        return Packet::packEncode($data, MainCmd::CMD_GAME, SubCmd::SUB_GAME_OUT_CARD);
    }
    ...
    protected function gameOutCardResp($serv, $account, $param)
    {
        ...
        //并提示成功
        return $this->gameOutCard(['status' => 0, 'msg' => '出牌成功', 'data' => $param]);
    }
}
